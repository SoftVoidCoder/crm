...
function outreachJourneyRecords(row) {
    ...
    const lead = leadId > 0
        ? (typeof crmLeadsDB !== 'undefined' ? crmLeadsDB : []).find(item => Number(item.id || 0) === leadId) || null
        : null;
    const dealId = Number(lead?.linked_deal_id || 0);
    const deal = leadId > 0
        ? (typeof crmDealsDB !== 'undefined' ? crmDealsDB : []).find(item => (dealId > 0 && Number(item.id || 0) === dealId) || Number(item.lead_id || 0) === leadId) || null
        : null;
    return { lead, deal };
}
